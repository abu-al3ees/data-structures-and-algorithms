...
const standardizePhoneNumbers = (arr) => {
  return arr.map(number => number.substring(1,4) + number.substring(6,9) + number.substring(10,14));
  // Solution code here...
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((character, i) => i % 2 === 1).join('');
  // Solution code here...
};
...
const allHappy = (arr) => {
  ...
  let newArray = arr.filter(string => string.includes(':)'));
  return length === newArray.length ? true : false;
  // Solution code here...
};
