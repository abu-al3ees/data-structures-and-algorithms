...
const toLastNames = people => {
  // Solution code here...
  const localArr = people.map( item => {
...
const validatePin = (pin) => {
  const pattren = /\b\d{4}\b/;
  return pattren.test(pin);
  // Solution code here...
};
...
const validateWord = (word) => {
  const pattren = /^[A-z]{5,10}$/;
  return pattren.test(word);
  // Solution code here...
};
...
const validateEmail = (email) => {
  const pattren = /^[\w\d]+\.?[\w\d]+?@[\w\d]+.(net|com|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  const pattren = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/;
  return pattren.test(phoneNumber);
  // Solution code here...
};
